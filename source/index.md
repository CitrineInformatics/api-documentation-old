...
from_page | If using pagination, set the index of starting record. Defaults to 0.
...
\* When searching over *all* data sets, at least one parameter other than _from_page_ or _per_page_ must be set.
...
start = 0
while size > 0:
  r = client.search(data_set_id='1', from_page=start, per_page=100)
  size = len(r.json())
  start += size
...
If you need to access more results than the number specified by _per_page_, then you can iterate through them by setting the _from_page_ parameter. The python example to the right shows the code to iterate through all records in a data set with id '1'.
